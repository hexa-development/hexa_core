...
Shared.Weapons = {
    ...
    ['weapon_revolver_cattleman_mexican'] = {
        ...
        label = 'Cattleman Mexican Revolver',
        weight = 3
    },
    ...
    ['weapon_revolver_doubleaction_gambler'] = {
        ...
        label = 'Double Action Gambler Revolver',
        weight = 3
    },
...
-- ============================================================
-- Shared.IsWeapon(name) -> boolean
-- ============================================================
-- ตัวตัดสินว่า "ชื่อนี้คืออาวุธหรือไม่" ที่ใช้ร่วมกันทั้งระบบ
-- สำคัญมากหลังย้ายมาเก็บแบบ esx_core เพราะ users.inventory กับ users.loadout
-- แบ่งกันด้วยฟังก์ชันนี้ตัวเดียว ถ้าฝั่งเซฟกับฝั่งโหลดตัดสินไม่ตรงกัน
-- อาวุธจะหายหรือถูกนับซ้ำทันที
--
-- *ห้ามเปลี่ยนไปเช็ค Shared.Items[name].type == 'weapon' แทน*
-- server/items.lua รวมอาวุธเข้า Shared.Items ให้แล้ว (type = 'weapon') แต่แคตตาล็อก
-- นั้นสร้างหลังต่อฐานข้อมูลเสร็จ ถ้าเผลอเซฟก่อนโหลดเสร็จอาวุธจะถูกมองเป็นของทั่วไป
-- แล้วโดนยุบเป็น {ชื่อ: จำนวน} = เสีย ammo/serial ถาวร
-- ตารางด้านบนเป็นข้อมูลนิ่งที่มีตั้งแต่วินาทีแรก จึงเชื่อถือได้ทุกจังหวะ
function Shared.IsWeapon(name)
    if type(name) ~= 'string' then return false end
    name = name:lower()
    -- แคตตาล็อกอาวุธด้านบนมาก่อน แล้วค่อย fallback เป็นชื่อขึ้นต้น weapon_
    -- (ครอบคลุมอาวุธที่เซิร์ฟเวอร์เพิ่มเองแต่ยังไม่ได้ใส่ในแคตตาล็อก)
    return Shared.Weapons[name] ~= nil or name:sub(1, 7) == 'weapon_'
end
